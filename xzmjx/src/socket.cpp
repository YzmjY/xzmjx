...
bool Socket::getOpt(int level,int optname,void* optval,socklen_t* optlen){
    ...
    if(rt != 0){
        XZMJX_LOG_DEBUG(g_logger)<<"getOpt sock="<<m_sock_fd
                                 <<" level="<<level<<" optname="<<optname<<" errno="
...
bool Socket::setOpt(int level,int optname,const void* optval,socklen_t optlen){
    ...
    if(rt != 0){
        XZMJX_LOG_DEBUG(g_logger)<<"setOpt sock="<<m_sock_fd
                                 <<" level="<<level<<" optname="<<optname<<" errno="
                                 <<errno<<" errstr="<<strerror(errno);
        return false;
    }
...
Socket::ptr Socket::accept(){
    ...
    if(client_sock == -1){
        XZMJX_LOG_ERROR(g_logger)<<" accept("<<m_sock_fd<<")errno="
                                 <<errno<<" errstr="<<strerror(errno);
        return nullptr;
    }
...
bool Socket::bind(const Address::ptr addr){
    ...
    if(::bind(m_sock_fd,addr->getAddr(),addr->getAddrLen())){
        XZMJX_LOG_ERROR(g_logger)<<" bind():sock_fd="<<m_sock_fd
...
bool Socket::listen(int backlog){
    ...
    if(::listen(m_sock_fd,backlog)){
        XZMJX_LOG_ERROR(g_logger)<<"listen error errno="
                                 <<errno<<" errstr="
                                 <<strerror(errno);
        return false;
    }
...
bool Socket::connect(const Address::ptr addr,uint64_t timeout_ms){
    ...
    if(timeout_ms == (uint64_t)-1){
        if(::connect(m_sock_fd,addr->getAddr(),addr->getAddrLen())) {
            XZMJX_LOG_ERROR(g_logger) << "connect() sock=" << m_sock_fd
                                      << " address=" << addr->toString()
                                      << " errno=" << errno
                                      << " errstr=" << strerror(errno);
            close();
...
bool Socket::cancelRead(){
    return IOManager::Self()->cancelEvent(m_sock_fd,IOManager::Event_READ);
}

bool Socket::cancelWrite(){
    return IOManager::Self()->cancelEvent(m_sock_fd,IOManager::Event_WRITE);
}

bool Socket::cancelAll(){
    return IOManager::Self()->cancelAll(m_sock_fd);
}
...
void Socket::newSock(){
    ...
    if(m_sock_fd != -1){
        ...
        XZMJX_LOG_ERROR(g_logger)<<"socket("<<m_family
