...
static xzmjx::ConfigVar<std::string>::ptr g_server_work_path = 
    xzmjx::Config::Lookup("server.work_path"
            ,std::string("/home/xzmjx/work/xzmjx")
            ,"server work path");

static xzmjx::ConfigVar<std::string>::ptr g_server_pid_file = 
    xzmjx::Config::Lookup("server.pid_file"
            ,std::string("xzmjx.pid")
            ,"server pid file");
...
bool Application::init(int argc,char**argv){
    ...
    if(!FSUtil::Mkdir(g_server_work_path->getValue())){
        XZMJX_LOG_FATAL(g_logger)<<"create work path ["<<
            g_server_work_path->getValue()<<" errno="<<errno
...
int Application::run_fiber(){
    ...
    for(auto&& i:modules){
        if(!i->onLoad()){
            XZMJX_LOG_ERROR(g_logger)<<"module name="
