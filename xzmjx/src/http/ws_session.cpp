...
xzmjx::ConfigVar<uint32_t>::ptr xzmjx::http::g_websocket_msg_max_size =
    xzmjx::Config::Lookup("websockt.message.max_size",(uint32_t)1024*1024*32,
...
WSFrameMessage::ptr WSRecvMessage(Stream* stream,bool client){
    ...
    do{
        ...
        if(ws_head.opcode == WSFrameHead::kPing){
        ...
        }else if(ws_head.opcode == WSFrameHead::kBin_Frame
               ||ws_head.opcode == WSFrameHead::kText_Frame
            ...
            if((cur_len+length)>=g_websocket_msg_max_size->getValue()){
                XZMJX_LOG_WARN(g_logger)<<"WSFrameMessage length > "
                    <<g_websocket_msg_max_size->getValue();
                break;
            }
            char mask[4];
            if(ws_head.mask){
