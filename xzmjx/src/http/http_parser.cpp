...
size_t HttpRequestParser::execute(char* data,size_t len){
    ...
    if(m_parser.upgrade){
        ...
        XZMJX_LOG_DEBUG(g_logger)<<"parser http request failed :"<< http_errno_description(HTTP_PARSER_ERRNO(&m_parser));
...
size_t HttpResponseParser::execute(char* data,size_t len){
    ...
    if(m_parser.upgrade){
        ...
        XZMJX_LOG_DEBUG(g_logger)<<"parser http response failed :"<< http_errno_description(HTTP_PARSER_ERRNO(&m_parser));
        setError(m_parser.http_errno);
    }else{
        if(n_parsered<len){
            memmove(data,data+n_parsered,len - n_parsered);
