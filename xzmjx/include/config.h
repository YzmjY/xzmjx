...
public:
 ...
 public:
     ...
     std::string toString() override{
        try{
            ...
            XZMJX_LOG_ERROR(XZMJX_LOG_ROOT())<<"ConfigVar::toString() exception "
                                             <<ex.what()<<" convert: "<<TypeToName<T>()<<" to string "
                                             <<"name = "<<m_name;
        }
        return "";
     }
     ...
     void fromString(const std::string& f) override{
        try{
            ...
            XZMJX_LOG_ERROR(XZMJX_LOG_ROOT())<<"ConfigVar::toString() exception "
                                             <<ex.what()<<" convert: string to " <<TypeToName<T>()
                                             <<"name = "<<m_name
                                             <<" - "<<f;
        }
     }
 ...
 private:
     ///@TODO 这里为什么要这么写，直接写两个静态的成员变量不行吗，还是说延迟初始化知道这两个变量真正被用到的时候
