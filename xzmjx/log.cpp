...
std::string ToString(LogLevel level) {
    ...
    switch (level) {
#define XX(name) \
    case LogLevel::name: ans = #name; break;
        XX(DEBUG)
        XX(INFO)
        XX(WARN)
        XX(ERROR)
...
LogEvent::LogEvent(std::shared_ptr<Logger> logger, LogLevel level, const char* filename, int32_t line, uint32_t elapse,
...
LogEventWrap::~LogEventWrap() { m_event->getLogger()->log(m_event->getLogLevel(), m_event); }

std::stringstream& LogEventWrap::getSS() { return m_event->getStringStream(); }
...
public:
    ...
    void format(std::ostream& os, Logger::ptr logger, LogLevel level, LogEvent::ptr event) override { os << std::endl; }
};

class StringFormatItem : public LogFormatter::FormatterItem {
public:
    StringFormatItem(const std::string& str) : m_string(str) {}
    void format(std::ostream& os, Logger::ptr logger, LogLevel level, LogEvent::ptr event) override { os << m_string; }
...
public:
    ...
    void format(std::ostream& os, Logger::ptr logger, LogLevel level, LogEvent::ptr event) override { os << "\t"; }
...
std::ostream& LogFormatter::format(std::ostream& ofs, std::shared_ptr<Logger> logger, LogLevel level,
                                   LogEvent::ptr event) {
    for (auto&& item : m_items) {
...
bool LogFormatter::error() { return m_error; }
/**
 * @brief 格式解析
 * @details
 *  %m 消息
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (m_pattern[i] != 'd') {
            ...
            if (index == std::string::npos) {
                m_items.push_back(
                    StringFormatItem::ptr(new StringFormatItem("<error format %d" + m_pattern.substr(i) + ">")));
                m_error = true;
                continue;
            } else {
                std::string fmt;
...
FileLogAppender::FileLogAppender(const std::string& filename) : m_filename(move(filename)) { reopen(); }
...
Logger::Logger(std::string name) : m_name(name), m_level(LogLevel::DEBUG) { m_formatter.reset(new LogFormatter); }
...
struct LogAppenderDefine {
    int type = 0; // 0:std 1:file
    LogLevel level = LogLevel::UNKONWN;
    std::string format;
    std::string file;

    bool operator==(const LogAppenderDefine& rhs) const {
        return type == rhs.type && level == rhs.level && format == rhs.format && file == rhs.file;
    }
};
...
struct LogDefine {
    ...
    bool operator==(const LogDefine& rhs) const {
        return name == rhs.name && level == rhs.level && format == rhs.format && appenders == appenders;
    }
...
public:
    std::string operator()(const LogDefine& define) {
        ...
        if (define.level != LogLevel::UNKONWN) {
            node["level"] = ToString(define.level);
        }
        for (auto&& it : define.appenders) {
            YAML::Node an;
            if (it.type == 0) {
                an["type"] = "StdoutLogAppender";
            } else if (it.type == 1) {
                an["type"] = "FileLogAppender";
                an["file"] = it.file;
            }
            if (it.level != LogLevel::UNKONWN) {
                an["level"] = ToString(it.level);
            }
            if (!it.format.empty()) {
                an["format"] = it.format;
            }
            node["appenders"].push_back(an);
        }
        std::stringstream ss;
        ss << node;
        return ss.str();
...
xzmjx::ConfigVar<std::set<LogDefine>>::ptr g_log_define =
    xzmjx::Config::Lookup("logs", std::set<LogDefine>(), "log config");
struct LogIniter {
    LogIniter() {
        g_log_define->addListener([](const std::set<LogDefine>& old_define, const std::set<LogDefine>& new_define) {
            for (auto&& define : new_define) {
                auto it = old_define.find(define);
                xzmjx::Logger::ptr logger;
                if (it == old_define.end()) {
                    logger = XZMJX_LOG_NAME(define.name);
                } else {
                    // name一致，可能变了也可能没变
                    if (!(*it == define)) {
                        logger = XZMJX_LOG_NAME(define.name);
                ...
                logger->setLevel(define.level);
                if (!define.format.empty()) {
                    logger->setFormatter(define.format);
                }
                logger->clearAppender();
                for (auto a : define.appenders) {
                    xzmjx::LogAppender::ptr appender;
                    if (a.type == 0) {
                    ...
                    if (!a.format.empty()) {
                        LogFormatter::ptr f(new LogFormatter(a.format));
                        if (!f->error()) {
                            appender->setFormatter(f);
                        } else {
                            std::cout << "log.name=" << define.name << " appender type=" << a.type
            ...
            for (auto&& it : old_define) {
                auto i = new_define.find(it);
                if (i == new_define.end()) {
                    LogMgr::GetInstance()->delLogger(it.name);
                }
            }
        });
    }
};
static LogIniter s_log_initer;
} // namespace xzmjx
