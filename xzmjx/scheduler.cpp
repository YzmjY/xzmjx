...
Scheduler::Scheduler(size_t thrNum, const std::string& name) : m_name(name), m_thr_num(thrNum) {
...
void Scheduler::start() {
    ...
    for (size_t i = 0; i < m_thr_num; ++i) {
        m_thread_pool[i].reset(new Thread([this] { this->run(); }, m_name + "_" + std::to_string(i)));
...
void Scheduler::notify() { XZMJX_LOG_INFO(g_logger) << "notify"; }

Scheduler* Scheduler::Self() { return t_scheduler; }
    ...
    while (true) {
        ...
        if (task.fiber) {
            ...
            if (task_fb->getState() == Fiber::FIBER_READY) {
                ...
                task_fb.reset();
            } else if (task_fb->getState() == Fiber::FIBER_TERM || task_fb->getState() == Fiber::FIBER_EXCEPT) {
...
void Scheduler::registerSelf() { t_scheduler = this; }
