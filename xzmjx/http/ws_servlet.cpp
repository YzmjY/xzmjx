...
namespace xzmjx {
namespace http {
WSServlet::WSServlet(const std::string& name) : Servlet(name) {}
...
int32_t FunctionWSServlet::onConnect(xzmjx::http::HttpRequest::ptr header, xzmjx::http::WSSession::ptr session) {
    if (m_onConnect) {
...
int32_t FunctionWSServlet::onClose(xzmjx::http::HttpRequest::ptr header, xzmjx::http::WSSession::ptr session) {
    if (m_onClose) {
...
int32_t FunctionWSServlet::handle(xzmjx::http::HttpRequest::ptr header, xzmjx::http::WSFrameMessage::ptr msg,
...
void WSServletDispatch::addServlet(const std::string& uri, WSServlet::callback cb, WSServlet::on_connect_cb connect_cb,
                                   WSServlet::on_close_cb close_cb) {
    ServletDispatch::addServlet(uri, std::make_shared<FunctionWSServlet>(cb, connect_cb, close_cb));
}

void WSServletDispatch::addGlobServlet(const std::string& uri, WSServlet::callback cb,
                                       WSServlet::on_connect_cb connect_cb, WSServlet::on_close_cb close_cb) {
