...
Semaphore::Semaphore(int count /*=0*/) { sem_init(&m_sem, 0, count); }

Semaphore::~Semaphore() { sem_destroy(&m_sem); }

void Semaphore::wait() { sem_wait(&m_sem); }

void Semaphore::notify() { sem_post(&m_sem); }

RWMutex::RWMutex() { pthread_rwlock_init(&m_rwmutex, NULL); }
...
bool Spinlock::tryLock() { return !(pthread_spin_trylock(&m_mutex)); }
