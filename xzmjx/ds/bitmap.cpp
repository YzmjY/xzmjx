...
Bitmap::Bitmap(uint32_t size, uint8_t def) : m_size(size), m_dataSize(ceil(1.0 * size / VALUE_SIZE)), m_data(nullptr) {
    m_data = (base_type*)malloc(m_dataSize * sizeof(base_type));
    memset(m_data, def, m_dataSize * sizeof(base_type));
}

Bitmap::Bitmap() : m_size(0), m_dataSize(0), m_data(nullptr) {}
...
std::string Bitmap::toString() const {
    std::stringstream ss;
    ss << "[size=" << m_size << " data_size=" << m_dataSize << " data=";
...
uint64_t countBits(const uint64_t& v) { return __builtin_popcount(v & 0xFFFFFFFF) + __builtin_popcount(v >> 32); }
...
bool Bitmap::operator!=(const Bitmap& rhs) const { return !(*this == rhs); }
