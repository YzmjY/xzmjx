...
public:
     ...
     * @brief 构造函数
     * @param cb
     * ：仿函数，可以通过bind绑定参数，从而实现线程可以执行有任何参数的函数体
     * @param name ：线程名称
     */
     ...
     * @return
     */
    uint64_t getThreadId() const { return m_tid; }
    ...
    /**
     * @brief
     * 设置线程名称，为一些不是我们创建的线程设置t_thread_name的值。例如main线程
     * @param name
     */
...
private:
     ...
     * @param arg
     * @brief
     * 这里设计为静态函数的原因有两个:1.符合pthread_create接口设计，非静态的成员函数有一个隐式的this指针参数.
