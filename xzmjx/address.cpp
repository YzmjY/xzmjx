...
Address::ptr Address::Create(const sockaddr *addr, socklen_t addrlen) {
    ...
    switch (addr->sa_family) {
        case AF_INET: result.reset(new IPv4Address(*(reinterpret_cast<const sockaddr_in *>(addr)))); break;
        case AF_INET6: result.reset(new IPv6Address(*(reinterpret_cast<const sockaddr_in6 *>(addr)))); break;
...
bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host, int family, int type, int protocol) {
    ...
    if (error) {
        XZMJX_LOG_ERROR(g_logger) << "Address::Lookup getaddress(" << host << "," << family << ", " << type
...
bool Address::GetInterfaceAddress(std::pair<Address::ptr, uint32_t> &result, const std::string &iface, int family) {
    return false;
}
...
IPAddress::ptr IPAddress::Create(const char *address, uint16_t port) {
    ...
    if (error) {
        XZMJX_LOG_ERROR(g_logger) << "IPAdress::Create(" << address << "," << port << ") error = " << error
...
IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port) {
    ...
    if (result <= 0) {
        XZMJX_LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", " << port << ") rt=" << result
                                  << " errno=" << errno << " errstr=" << strerror(errno);
        return nullptr;
    }
    return rt;
...
std::ostream &IPv4Address::insert(std::ostream &out) {
    ...
    out << ((addr >> 24) & 0xff) << ":" << ((addr >> 16) & 0xff) << ":" << ((addr >> 8) & 0xff) << ":" << (addr & 0xff);
    out << ":" << EndianCastOnLittle(m_address.sin_port);
    return out;
}

uint32_t IPv4Address::getPort() const { return EndianCastOnLittle(m_address.sin_port); }

void IPv4Address::setPort(uint16_t port) { m_address.sin_port = EndianCastOnLittle(port); }
...
IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port) {
    ...
    if (error <= 0) {
        XZMJX_LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", " << port << ") rt=" << error
                                  << " errno=" << errno << " errstr=" << strerror(errno);
        return nullptr;
    }
    return ans;
...
uint32_t IPv6Address::getPort() const { return EndianCastOnLittle(m_address.sin6_port); }

void IPv6Address::setPort(uint16_t port) { m_address.sin6_port = EndianCastOnLittle(port); }
...
const sockaddr *UnknownAddress::getAddr() const { return (sockaddr *)&m_address; }
