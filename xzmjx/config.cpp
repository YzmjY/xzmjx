...
void ListAllNodes(std::string prefix, YAML::Node node, std::list<std::pair<std::string, YAML::Node>>& ouput) {
    ...
    if (node.IsMap()) {
        for (auto it = node.begin(); it != node.end(); it++) {
            ListAllNodes(prefix == "" ? prefix = it->first.Scalar() : prefix + "." + it->first.Scalar(), it->second,
                         ouput);
        }
    }
}
...
void Config::LoadFromConfDir(const std::string& path, bool force) {
    ...
    for (auto&& file : files) {
        try {
            ...
            LoadFromYaml(n);
            XZMJX_LOG_INFO(g_logger) << "LoadConfFile file=" << file << " ok";
        } catch (...) {
            XZMJX_LOG_ERROR(g_logger) << "LoadConfFile file=" << file << " fali";
        }
    }
}
