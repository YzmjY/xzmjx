...
namespace xzmjx {
bool IsHookEnable() { return t_enable_hook; }
void SetHookEnable(bool hook_flag) { t_enable_hook = hook_flag; }
void EnableHook() { t_enable_hook = true; }
} // namespace xzmjx
...
/**
 * @brief
 * 利用在main函数之前执行静态变量的初始化特性，完成在进入main之前保存hook的原函数
 */
...
static ssize_t do_io(int fd, OriginFun fun, const char *hook_fun_name, uint32_t event, int timeout_so, Args &&...args) {
    ...
    if (!fd_ctx->isSocket() || fd_ctx->getUserNonblock()) {
        ///@details
        /// 用户设置了非阻塞，则用户需要自己为可能发生的读不全结果负责，不hook
        return fun(fd, std::forward<Args>(args)...);
    }
...
retry:
    ///@details
    /// 用户未设置为非阻塞，但实际均为非阻塞，利用hook后加入epoll或定时器来实现非阻塞的效果。
    ...
    if (n == -1 && errno == EAGAIN) {
        ...
        if (to != (uint64_t)-1) {
            ///@details 设置有超时时间
            timer = iom->addCondTimerEvent(
                to,
                [weak_cond, fd, iom, event]() {
...
unsigned int sleep(unsigned int seconds) {
    ...
    xzmjx::IOManager *iom = xzmjx::IOManager::Self();
    iom->addTimerEvent(seconds * 1000, [&iom, &fiber]() { iom->submit(fiber); });
    xzmjx::Fiber::YieldToHold();
    return 0;
}
...
int nanosleep(const struct timespec *req, struct timespec *rem) {
    ...
    iom->addTimerEvent(timeout_ms, [&iom, &fiber]() { iom->submit(fiber); });
    xzmjx::Fiber::YieldToHold();
    return 0;
}
...
ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen) {
    return do_io(sockfd, recvfrom_f, "recvfrom", xzmjx::IOManager::Event_READ, SO_RCVTIMEO, buf, len, flags, src_addr,
                 addrlen);
}
...
int connect_with_timeout(int fd, const struct sockaddr *addr, socklen_t addrlen, uint64_t timeout_ms) {
    ...
    if (timeout_ms != (uint64_t)-1) {
        timer = iom->addCondTimerEvent(
            ...
            [winfo, fd, iom]() {
                ...
                ///@details
                /// 如果超时，这里会触发一次事件，结果是该协程从yield点后返回
    ...
    ///@details
    /// 添加监听事件，等待超时或者connect成功，超时会由于cancelEvent触发写事件将fiber加入调度
