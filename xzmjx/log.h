...
#define XZMJX_LOG_LEVEL(logger, level)                                                                       \
    ...
    xzmjx::LogEventWrap(xzmjx::LogEvent::ptr(new xzmjx::LogEvent(                                            \
                            logger, level, __FILE__, __LINE__, 0, xzmjx::GetThreadID(), xzmjx::GetFiberID(), \
                            static_cast<uint64_t>(time(NULL)), xzmjx::Thread::GetName())))                   \
        .getSS()
...
enum class LogLevel { UNKONWN = 0, DEBUG = 1, INFO = 2, WARN = 3, ERROR = 4, FATAL = 5 };
...
public:
    ...
    LogEvent(std::shared_ptr<Logger> logger, LogLevel level, const char* filename, int32_t line, uint32_t elapse,
    ...
    std::stringstream& getStringStream() { return m_sstream; }
