...
static std::ostream& operator<<(std::ostream& os, const EpollCtlOp& op) {
    switch (op) {
        case EPOLL_CTL_ADD: return os << "EPOLL_CTL_ADD";
        case EPOLL_CTL_DEL: return os << "EPOLL_CTL_DEL";
        case EPOLL_CTL_MOD: return os << "EPOLL_CTL_MOD";
        default: return os << static_cast<int>(op);
    }
}
...
IOManager::IOManager(size_t thrNum, const std::string& name) : Scheduler(thrNum, name) {
...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if (fd_ctx->events & event) {
        XZMJX_LOG_ERROR(g_logger) << "addEvent assert fd=" << fd << " event=" << static_cast<EPOLL_EVENTS>(event)
                                  << " fd_ctx.event=" << static_cast<EPOLL_EVENTS>(fd_ctx->events);
        XZMJX_ASSERT(!(fd_ctx->events & event));
    }
    int op = fd_ctx->events ? EPOLL_CTL_MOD : EPOLL_CTL_ADD;
    epoll_event epevent;
    epevent.events = uint32_t(EPOLLET) | uint32_t(fd_ctx->events | event);
    epevent.data.ptr = fd_ctx;

    int rt = epoll_ctl(m_epoll_fd, op, fd, &epevent);
    if (rt) {
        XZMJX_LOG_ERROR(g_logger) << "epoll_ctl" << "(" << m_epoll_fd << "," << static_cast<EpollCtlOp>(op) << "," << fd
                                  << static_cast<EPOLL_EVENTS>(epevent.events) << "):ret = " << rt << " (" << errno
    ...
    XZMJX_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
...
bool IOManager::delEvent(int fd, Event event) {
    ...
    epoll_event epevent;
    epevent.events = uint32_t(EPOLLET) | uint32_t(new_events);
    epevent.data.ptr = fd_ctx;
...
Timer::ptr IOManager::addCondTimerEvent(uint64_t ms, std::function<void()> cb, std::weak_ptr<void> cond,
                                        bool recurring) {
    return m_time_manager->addCondTimer(ms, std::move(cb), cond, recurring);
}

IOManager* IOManager::Self() { return dynamic_cast<IOManager*>(Scheduler::Self()); }
...
void IOManager::wait() {
    ...
    while (true) {
        ...
        for (int i = 0; i < rt; ++i) {
            ...
            if (rt2) {
                XZMJX_LOG_ERROR(g_logger)
                    << "epoll_ctl" << "(" << m_epoll_fd << "," << static_cast<EpollCtlOp>(op) << "," << fd_ctx->fd
                    << "," << static_cast<EPOLL_EVENTS>(event.events) << "):ret = " << rt << " (" << errno << ") ("
                    << strerror(errno) << ") fd_ctx->events=" << (EPOLL_EVENTS)fd_ctx->events;
                continue;
            }
...
bool IOManager::canStop(uint64_t& next_timer) {
    ...
    return next_timer == ~0ull && m_pending_event_count == 0 && Scheduler::canStop();
}
