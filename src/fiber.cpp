...
namespace xzmjx{
    ...
    /**
     * @details 这里t_fiber使用原生指针的目的在于:不管协程模块外面是如何的使用，都不能出现由于协程的切换而导致该协程的析构
     *          而使用智能指针，则可能出现切换协程时，引用计数减至0，导致该协程析构,但我觉得不会，所以没管
     *
     */
    static thread_local Fiber::ptr t_fiber;             ///该线程上当前正在跑的协程，每次切换协程要改变这个值
    static thread_local Fiber::ptr t_mainFiber;         ///该线程上的主协程，即线程对应的协程，每次切换协程都以该协程为出发点和目的地
    ...
    Fiber::~Fiber(){
        ...
        --s_fiberCount;
        if(m_stack){
            XZMJX_ASSERT(m_state==Fiber::FIBER_INIT||
                        m_state==Fiber::FIBER_EXCEPT||
                        m_state==Fiber::FIBER_TERM);
    ...
    uint64_t Fiber::GetFiberId(){
        ...
        ///@brief 只可能出现在Fiber::Fiber()中，此时还没有InitialMianFiber，所以导致t_fiber未赋值
        return 0;
    }

    uint64_t Fiber::TotalCount(){
        return s_fiberCount;
    }
